...
export const StatsModal: Component = () => {
  ...
  const handleOpenChange = (open: boolean) => {
    if (!open) closeStatsModal();
  };

  return (
    <Dialog open={showStatsModal()} onOpenChange={handleOpenChange}>
      <DialogContent class="p-0 overflow-hidden sm:rounded-3xl w-full max-w-6xl max-h-[85vh] border-border bg-popover/90 backdrop-blur-xl shadow-2xl flex flex-col">
          ...
          <div class="bg-secondary/20 border border-border/50 rounded-2xl p-5 flex flex-col gap-3 backdrop-blur-sm">
            ...
            <div class="flex h-4 w-full bg-secondary/50 rounded-full overflow-hidden mt-2 border border-border/30">
              <Show when={sidebarCounts().all > 0}>
                <div class="flex items-center justify-center text-[9px] font-bold text-white leading-none transition-all duration-300" style={{ width: `${(sidebarCounts().downloading / sidebarCounts().all) * 100}%`, background: 'var(--color-primary-500)' }} title={t('stats.status_downloading', { n: sidebarCounts().downloading })}>{sidebarCounts().downloading > 0 && (sidebarCounts().downloading / sidebarCounts().all) * 100 >= 5 ? sidebarCounts().downloading : ''}</div>
                <div class="flex items-center justify-center text-[9px] font-bold text-white leading-none transition-all duration-300" style={{ width: `${(sidebarCounts().seeding / sidebarCounts().all) * 100}%`, background: 'var(--color-success-500)' }} title={t('stats.status_seeding', { n: sidebarCounts().seeding })}>{sidebarCounts().seeding > 0 && (sidebarCounts().seeding / sidebarCounts().all) * 100 >= 5 ? sidebarCounts().seeding : ''}</div>
                <div class="flex items-center justify-center text-[9px] font-bold text-white leading-none transition-all duration-300" style={{ width: `${(sidebarCounts().stopped / sidebarCounts().all) * 100}%`, background: '#6b7280' }} title={t('stats.status_stopped', { n: sidebarCounts().stopped })}>{sidebarCounts().stopped > 0 && (sidebarCounts().stopped / sidebarCounts().all) * 100 >= 5 ? sidebarCounts().stopped : ''}</div>
                <div class="flex items-center justify-center text-[9px] font-bold text-white leading-none transition-all duration-300" style={{ width: `${(sidebarCounts().checking / sidebarCounts().all) * 100}%`, background: 'var(--color-warning-500)' }} title={t('stats.status_checking', { n: sidebarCounts().checking })}>{sidebarCounts().checking > 0 && (sidebarCounts().checking / sidebarCounts().all) * 100 >= 5 ? sidebarCounts().checking : ''}</div>
                <div class="flex items-center justify-center text-[9px] font-bold text-white leading-none transition-all duration-300" style={{ width: `${(sidebarCounts().error / sidebarCounts().all) * 100}%`, background: 'var(--color-danger-500)' }} title={t('stats.status_error', { n: sidebarCounts().error })}>{sidebarCounts().error > 0 && (sidebarCounts().error / sidebarCounts().all) * 100 >= 5 ? sidebarCounts().error : ''}</div>
                <div class="flex items-center justify-center text-[9px] font-bold text-white leading-none transition-all duration-300" style={{ width: `${(sidebarCounts().queued / sidebarCounts().all) * 100}%`, background: '#8b5cf6' }} title={t('stats.status_queued', { n: sidebarCounts().queued })}>{sidebarCounts().queued > 0 && (sidebarCounts().queued / sidebarCounts().all) * 100 >= 5 ? sidebarCounts().queued : ''}</div>
              </Show>
            </div>
