...
export const AddTorrentModal: Component = () => {
  ...
  const availableLabels = () => {
    ...
    try {
      const stored = localStorage.getItem('trwm-label-library');
      if (stored) {
  ...
  return (
    <Dialog open={showAddModal()} onOpenChange={handleOpenChange}>
      <DialogContent class="p-0 overflow-hidden w-[95%] max-w-2xl bg-background/95 backdrop-blur-xl border border-white/10 shadow-2xl rounded-xl">
        <DialogHeader class="px-4 py-3 border-b border-border/50 bg-secondary/30">
        ...
        <form onSubmit={handleAdd} class="flex flex-col flex-1 overflow-y-auto max-h-[75vh] p-4 gap-4">
          ...
          <div class="space-y-3">
              ...
              <div class="space-y-1.5">
                ...
                <Select
                  ...
                  value={[
                    ...
                    { value: 0, label: t('detail.settings.priority_normal') },
                    { value: 1, label: t('detail.settings.priority_high') },
                  ].find((o) => o.value === priority())}
                  onChange={(v: any) => v && setPriority(v.value ?? v)}
                  disabled={adding()}
                  itemComponent={(props) => <>{props.item.label}</>}
                  renderValue={(v) => <>{v.label}</>}
                  triggerClass={cn(inputClass, "p-0 px-2.5")}
                />
              </div>
              <div class="space-y-1.5">
                ...
                <Select
                  options={[
                    { value: '', label: t('dialog.add.group_default') },
                    ...(groupsData.isSuccess ? groupsData.data.map(g => ({ value: g.name, label: g.name })) : [])
                  ]}
                  optionValue="value"
                  optionTextValue="label"
                  value={[
                    { value: '', label: t('dialog.add.group_default') },
                    ...(groupsData.isSuccess ? groupsData.data.map(g => ({ value: g.name, label: g.name })) : [])
                  ].find((o) => o.value === group())}
                  onChange={(v: any) => v && setGroup(v.value ?? v)}
            ...
            <div class="flex items-center gap-6 pt-2">
              ...
              <Checkbox class="text-primary border-primary/50 focus-visible:ring-primary/20" checked={sequential()} onChange={(checked) => setSequential(checked)} disabled={adding()}>
        ...
        <DialogFooter class="flex items-center justify-end gap-2 px-4 py-3 border-t border-border/50 bg-secondary/20 sm:space-x-0">
