// @ts-nocheck
    ...
    function lookup(ipStr) {
        ...
        var code = (data && data.country && data.country.iso_code) ||
            (data && data.registered_country && data.registered_country.iso_code) ||
            (data && data.continent && data.continent.code);

        if (code) {
            var name = _countryNames[code.toLowerCase()] || code;
    ...
    function getCountryDisplayHtml(ip) {
        if (is_privateIP(ip)) {
            return '<span style="font-size:10px;background:var(--bg-tertiary);padding:0 3px;border-radius:2px;color:var(--text-muted)">LAN</span>';
        }
        var info = lookup(ip);
        if (info) {
            var flagHtml = getCountryFlagHtml(info.code);
