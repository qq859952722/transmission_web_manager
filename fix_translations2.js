...
function fixFile(filePath, lang) {
  ...
  const getTranslation = (key) => {
    if (lang === 'zh') {
      ...
      if (key === 'incomplete_enabled') return '启用临时目录';
      if (key === 'start_added') return '添加后立即开始';
      if (key === 'rename_partial') return '未完成的文件加后缀';
      if (key === 'trash_torrent') return '删除原始种子文件';
      if (key === 'alt_speed_time') return '启用定时备用限速';
      ...
      if (key === 'global_peer_limit') return '全局最大连接数';
  ...
  // Find dialog block
  const dialogIdx = content.indexOf('"dialog":');
  if (dialogIdx === -1) return;
  
  // Find settings block AFTER dialog block
  const settingsIdx = content.indexOf('"settings": {', dialogIdx);
  if (settingsIdx === -1) return;
  
  const insertPos = settingsIdx + '"settings": {'.length;
